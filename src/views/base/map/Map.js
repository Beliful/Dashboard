...
import iotDevices from 'src/data/iotDevice'; // Assuming you have IOT device data
...
const Map = () => {
  ...
  return (
    <div style={{ height: '500px' }}> {/* Ensure the container has a height */}
      <button onClick={toggleDevices}>{showTractors ? 'Show IOT Devices' : 'Show Tractors'}</button>
      <MapContainer center={[39, 35]} zoom={6} scrollWheelZoom={false} style={{ height: '100%' }}> {/* Set height for the MapContainer */}
        <TileLayer
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a>'
        />
        {devices.map(device => (
          <Marker key={device.id} position={[device.location.latitude, device.location.longitude]} icon={showTractors ? iconTruck : iconIOT}>
            <Popup>
              {device.name}
            </Popup>
          </Marker>
